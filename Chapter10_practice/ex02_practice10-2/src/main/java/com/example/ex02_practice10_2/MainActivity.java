...
public class MainActivity extends AppCompatActivity {
    ...
    String[] imageNames = {"이미지1", "이미지2", "이미지3", "이미지4",     //이미지들의 이름을 저장한 배열
        ...
        for (int i = 0; i < imageViews.length; i++) {
            ...
            imageViews[i].setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    voteCount[index]++;
                    Toast.makeText(MainActivity.this, imageNames[index]+"총"+voteCount[index]+"표",Toast.LENGTH_LONG).show();
                }
            });
        }

        // [투표 종료] 버튼 이벤트 처리
        // 이미지들의 이름과 튜표 결과 배열을 ResultActivity로 전송
